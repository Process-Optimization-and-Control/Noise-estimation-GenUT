...
class UnscentedKalmanFilter(object):
    ...
    def __init__(self, dim_x, dim_z, dt, hx, fx, points,
        ...
        if sqrt_fn is None:
            self.msqrt = scipy.linalg.cholesky
        else:
            self.msqrt = sqrt_fn
    ...
    def predict(self, dt=None, UT=None, fx=None, **fx_args):
        ...
        **fx_args : keyword arguments
        ...
        self.x, self.P = UT(self.sigmas_f, self.Wm, self.Wc, self.Q, #here, sigmas_f = fx(sigma points) - each point has gone through the nonlinear transformation already
                            self.x_mean, self.residual_x)
        # print("ok2")
        # update sigma points to reflect the new variance of the points
        self.sigmas_f = self.points_fn.sigma_points(self.x, self.P) #these sigma points have not gone through any transformation - it is just the pure sigma points (not f(sigma points))
        # print("ok3")
        # save prior
        self.x_prior = np.copy(self.x)
        self.P_prior = np.copy(self.P)
...
class UnscentedKalmanFilterParametricUncertainty(UnscentedKalmanFilter):
    def __init__(self, dim_par, dim_x, dim_z, dt, hx, fx, points, name = None):
        """
        Init
        ...
        ----------
        dim_par : TYPE int
            DESCRIPTION. Number of parameters (uncertain distribution, but the distribution is fixed at every time step)
        dim_x : TYPE int
            DESCRIPTION. Number of states to estimate
        dim_z : TYPE int
            DESCRIPTION. See parent class
        dt : TYPE float
            DESCRIPTION. See parent class
        hx : TYPE function
            DESCRIPTION. See parent class
        fx : TYPE function 
            DESCRIPTION. See parent class
        points : TYPE ParametricUncertaintyGenUTSigmaPoints
            DESCRIPTION. Generalized sigma points which utilizes that par is a fixed distribution and x is assumed to be Gaussian. Should probably do an input check that the type is correct (not implemented yet)
        ...
        self.dim_x0 = dim_x
        self.dim_xa = dim_par + dim_x
        super().__init__(self.dim_xa, dim_z, dt, hx, fx, points, name = name)
        
    def compute_process_sigmas(self, dt, fx=None, **fx_args):
        """
        Same as parent class, except that self.Wm, self.Wc is updated
        """
        ...
        # The weights change at every time step, since P_xx ==>cm4_x changes. Need to update them here
    ...
    def predict(self, dt=None, UT=None, fx=None, **fx_args):
        ...
        **fx_args : keyword arguments
        ...
        # update sigma points to reflect the new variance of the points
        self.sigmas_f = self.points_fn.sigma_points(self.x, self.P) #these sigma points have not gone through any transformation - it is just the pure sigma points (not f(sigma points))
        self.Wm, self.Wc = self.points_fn.Wm, self.points_fn.Wc #with the genUT the weights can  change since they are dependent on the scaling parameters (which are dependent of self.P)
        
        # save prior
        self.x_prior = np.copy(self.x)
        self.P_prior = np.copy(self.P)
        
class JointGenUTFilter(UnscentedKalmanFilter):
    def __init__(self, dim_par, dim_x, dim_z, dt, hx, fx, points, name = None):
        """
        Init
    ...
    def predict(self, dt=None, UT=None, fx=None, **fx_args):
        ...
        **fx_args : keyword arguments
        ...
        #Here, we compute w ~(w_mean, Q) and NOT x, P. Move therefore these lines to the main part of the code.
        # update sigma points to reflect the new variance of the points
        # self.sigmas_f = self.points_fn.sigma_points(self.x, self.P) #these sigma points have not gone through any transformation - it is just the pure sigma points (not f(sigma points))
        # self.Wm, self.Wc = self.points_fn.Wm, self.points_fn.Wc #with the genUT the weights can  change since they are dependent on the scaling parameters (which are dependent of self.P)
        ...
        return self.x, self.P #this is actually w_mean, Qkin this case
